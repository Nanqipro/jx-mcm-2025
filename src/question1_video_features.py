...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="从雾天视频中提取图像清晰度特征")
    parser.add_argument("--video", type=Path, required=True, help="输入视频文件")
    parser.add_argument("--interval", type=float, default=60.0, help="抽帧间隔（秒）")
    parser.add_argument(
        "--method",
        choices=("frame", "time"),
        default="frame",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=repo_root / "results" / "generated" / "fog-analysis.csv",
        help="特征结果 CSV",
    )
    return parser
...
def main():
    ...
    try:
        # 创建分析器
        analyzer = SimpleFogVideoAnalyzer(str(video_path))

        # 执行快速分析
        # interval_seconds: 每隔多少秒提取一帧
        # method: 'frame'(按帧跳转) 或 'time'(按时间跳转)
        results = analyzer.quick_analysis(interval_seconds=args.interval, method=args.method)

        if results is not None:
            # 保存结果到指定目录
            analyzer.save_results(str(args.output))
