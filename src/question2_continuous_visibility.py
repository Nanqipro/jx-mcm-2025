...
class ContinuousVisibilityModel:
    ...
    def __init__(self, results_dir: Optional[Union[str, Path]] = None):
        ...
        self.results_dir = str(Path(results_dir) if results_dir else repo_root / "results" / "generated")
...
def main():
    ...
    repo_root = Path(__file__).resolve().parents[1]
    parser = argparse.ArgumentParser(description="建立能见度随时间连续变化模型")
    parser.add_argument(
        "--data",
        type=Path,
        default=repo_root / "data" / "private" / "processed" / "blur.csv",
        help="含 AMOS 能见度与气象字段的 CSV 文件",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=repo_root / "results" / "generated",
        help="生成图表和报告的目录",
    )
    ...
    model = ContinuousVisibilityModel(args.output_dir)
    print(f"📁 使用数据文件: {data_path}")
    
    try:
        # 运行完整分析
        results = model.run_complete_analysis(str(data_path), prediction_steps=120)
