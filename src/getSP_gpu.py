        ...
        if self.use_gpu:
            try:
                ...
                if h > 2 and w > 2:
                    ...
                    # 中心区域计算
                    laplacian_gpu[1:-1, 1:-1] = (
                        gray_gpu[:-2, 1:-1] + gray_gpu[2:, 1:-1] + 
                        gray_gpu[1:-1, :-2] + gray_gpu[1:-1, 2:] - 
                        4 * gray_gpu[1:-1, 1:-1]
                    )
                else:
                    laplacian_gpu = gray_gpu
                ...
                print(f"⚠️ GPU计算失败，回退到CPU: {e}")
                # 自动回退到CPU计算
                self.use_gpu = False
                return self.extract_sharpness_features_gpu(image)
    ...
    def extract_contrast_features_gpu(self, image: np.ndarray) -> dict:
        ...
        if self.use_gpu:
            try:
                ...
                print(f"⚠️ GPU对比度计算失败，回退到CPU: {e}")
                self.use_gpu = False
                return self.extract_contrast_features_gpu(image)
    ...
    def extract_frequency_features_gpu(self, image: np.ndarray) -> dict:
        ...
        if self.use_gpu:
            try:
                ...
                print(f"⚠️ GPU频域计算失败，回退到CPU: {e}")
                self.use_gpu = False
                return self.extract_frequency_features_gpu(image)
